...
OUT = Path(os.environ.get("SECTORAL_POSITIONS_PHASE_B_AUDIT_OUT","sectoral_positions_phase_b_audit_artifacts"))
OUT.mkdir(parents=True, exist_ok=True)
API = "https://data-api.ecb.europa.eu/service/data/QSA/"
USER_AGENT = "romanian-monetary-dynamics/0.1.0 (+GitHub sectoral-financial-positions Phase B audit)"
TOL = 0.1
...
def load_contract() -> dict:
    return json.loads((ROOT/"model/dynamics/sectoral_financial_positions_aggregate_identity_contract.json").read_text(encoding="utf-8"))
...
def main() -> None:
    ...
    report={"audit_version":"0.1","phase":"Reference Mode Recovery — Sectoral Financial Positions / Phase B exact aggregate identity","reference_mode":"sectoral_financial_positions","contract":"model/dynamics/sectoral_financial_positions_aggregate_identity_contract.json","source":{"series_requested":len(required),"series_status_counts":dict(Counter(str(x.get('status')) for x in series.values())),"network_errors_present":network,"unavailable_mandatory_series":unavailable},"construction_errors":errors,"measure_assessments":assessments,"sector_series":sectors,"source_results":series,"promotion_eligible":eligible,"accounting_spine_changed":False,"accounting_readiness_changed":False,"bilateral_materialization":False,"behavioural_closure_changed":False,"promotion_status":"AGGREGATE_SECTORAL_REFERENCE_EVIDENCE_READY_FOR_REPOSITORY_PROMOTION_GATE" if eligible else "REFERENCE_MODE_REMAINS_PARTIAL_SERIES_AVAILABLE"}
    (OUT/"sectoral_financial_positions_phase_b_audit.json").write_text(json.dumps(report,indent=2,ensure_ascii=False)+"\n",encoding="utf-8")
    print(json.dumps({"series_requested":len(required),"series_status_counts":report["source"]["series_status_counts"],"unavailable_mandatory_series_count":len(unavailable),"construction_errors":errors,"measure_assessments":assessments,"promotion_eligible":eligible,"promotion_status":report["promotion_status"]},indent=2))
    if not eligible:
