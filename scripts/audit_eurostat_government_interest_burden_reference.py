...
OUT = Path(
    os.environ.get(
        "GOV_INTEREST_REFERENCE_AUDIT_OUT",
        "government_interest_reference_audit_artifacts",
    )
)
...
USER_AGENT = (
    ...
    "(+GitHub government-interest reference-mode audit)"
)
...
def fetch_unit(unit: str, contract: dict) -> dict[str, object]:
    ...
    raw_path = OUT / "raw" / f"gov_10q_ggnfa_D41PAY_RO_{unit}.json"
...
def main() -> None:
    ...
    actual_units = set(results)
    all_units_present = actual_units == expected_units
    promotion_eligible = (
        all_units_present
    ...
    report = {
        "audit_version": "0.1",
        "phase": "Reference Mode Recovery — Government Interest Burden",
        "reference_mode": "government_interest_burden",
        ...
        "government_refinancing_mechanism_status_changed": False,
        "government_effective_rate_derived": False,
        "promotion_status": (
            "OBSERVED_SERIES_EVIDENCE_READY_FOR_REPOSITORY_ASSESSMENT"
            if promotion_eligible
            else "REFERENCE_MODE_REMAINS_SOURCE_IDENTIFIED_NOT_MATERIALIZED"
        ),
        "interpretation": (
            "D41PAY is retained only as ESA general-government interest "
            "expenditure. MIO_NAC provides the nominal expenditure reference "
            "mode and PC_GDP a normalized burden reference. Neither series is "
            "an effective interest rate, marginal yield, cash payment measure, "
            "or refinancing-share estimate."
        ),
    }

    report_path = OUT / "government_interest_burden_reference_audit.json"
    ...
    if not promotion_eligible:
        raise SystemExit(
            "Government-interest reference-mode source gate did not pass; "
