...
OUT = Path(os.environ.get("F5_AUDIT_OUT", "f5_audit_artifacts"))
OUT.mkdir(parents=True, exist_ok=True)

API = "https://data-api.ecb.europa.eu/service/data/QSA/"
USER_AGENT = "romanian-monetary-dynamics/0.1.0 (+GitHub F5 coverage audit)"
...
def main() -> None:
    ...
    for measure in ("LE", "F"):
        for holder in SECTORS:
            for issuer in SECTORS:
                formulas = candidate_terms(holder, issuer, measure, "F5")
    ...
    for measure in ("LE", "F"):
        for sector in RESIDENT:
            for kind, entry in (("holder_total", "A"), ("issuer_total", "L")):
                terms = aggregate_terms(sector, entry, measure, "F5")
    ...
    for measure, kind, sector, terms in controls:
        ...
        complete = all(c["status"] == "OBSERVABLE_OR_EXACT_DERIVATION" for c in related)
    ...
    for measure, sector, entry, by_instrument in decomposition_controls:
        ...
        if all(vals[k] is not None for k in ("F5", "F51", "F52")):
            residual = float(vals["F5"]) - float(vals["F51"]) - float(vals["F52"])
        ...
        decomposition.append({
            ...
            "sector": sector,
            "entry": entry,
            "F5_million_RON": vals["F5"],
            "F51_million_RON": vals["F51"],
            "F52_million_RON": vals["F52"],
            "residual_F5_minus_F51_minus_F52_million_RON": residual,
            "status": status,
            "terms": details,
        })
    ...
    report = {
        ...
        "network_errors_present": any(x.get("status") == "NETWORK_ERROR" for x in series_by_key.values()),
        "cells": cells,
        "aggregate_reconciliation": reconciliation,
        "total_economy_controls": totals,
        "sector_F5_equals_F51_plus_F52_controls": decomposition,
        "total_F5_equals_F51_plus_F52_controls": total_decomposition,
        "rule": (
            "Coverage only. F5 is the total equity-and-investment-fund instrument. "
            "F51 equity and F52 investment fund shares/units are decomposition controls; "
            "neither substitutes for F5. QSA valuation and non-consolidated dimensions "
            "are kept explicit in every source key."
        ),
    }

    (OUT / "f5_equity_fund_coverage_audit.json").write_text(
