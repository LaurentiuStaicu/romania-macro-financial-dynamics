...
def layout_audit(page,product:str)->dict:
    ...
    return page.evaluate("""({primarySelector})=>{
      ...
      const smallControls=[...document.querySelectorAll('button,select,input:not([type="checkbox"]):not([type="radio"])')].filter(visible).map(el=>({label:(el.textContent||el.getAttribute('aria-label')||el.id||el.tagName).trim().slice(0,80),h:el.getBoundingClientRect().height,w:el.getBoundingClientRect().width})).filter(x=>x.h<24||x.w<24);
...
def computed_contract(page,product:str)->dict:
    ...
    return page.evaluate("""({primary})=>{
      ...
      const rect=s=>{const r=document.querySelector(s).getBoundingClientRect();return {height:r.height}};
      const body=css('body'),header=css('.product-header'),brand=css('.brand'),img=css('.brand img'),h1=css('.brand h1'),sub=css('.brand p'),lang=css('.lang'),active=css('.lang button[aria-pressed="true"]'),main=css('main'),q=css('.question-band'),panel=css(primary),quiet=css('.quiet');
      const pack=(s,keys)=>Object.fromEntries(keys.map(k=>[k,s[k]]));
      return{body:pack(body,['fontFamily','backgroundColor','color']),header:{...pack(header,['minHeight','paddingTop','paddingRight','paddingBottom','paddingLeft','gap','backgroundColor','borderBottomColor','backdropFilter']),...rect('.product-header')},brand:pack(brand,['gap']),icon:pack(img,['width','height']),title:pack(h1,['fontSize']),subtitle:pack(sub,['fontSize','color']),language:pack(lang,['borderTopColor','borderTopWidth','borderRadius']),languageActive:{...pack(active,['backgroundColor','color','paddingTop','paddingRight','paddingBottom','paddingLeft']),...rect('.lang button[aria-pressed="true"]')},main:pack(main,['maxWidth','paddingTop','paddingRight','paddingBottom','paddingLeft']),questionBand:pack(q,['minHeight','marginBottom','borderBottomColor']),primaryPanel:pack(panel,['backgroundColor','borderTopColor','borderRadius','boxShadow']),quiet:{...pack(quiet,['backgroundColor','color','borderTopColor','borderRadius','paddingTop','paddingRight','paddingBottom','paddingLeft']),...rect('.quiet')}}
    }""",{"primary":primary})
        ...
        try:
            for width,height in VIEWPORTS:
                ...
                for name,audit in (("Macro",maudit),("World3",waudit)):
                    ...
                    if audit['smallControls']:raise SystemExit(f"{name} controls below 24px target floor at {label}: {audit['smallControls']}")
                ...
                side_by_side(world_path,macro_path,side_dir/f"{label}.png",label);report['viewports'][label]={"macro":maudit,"world3":waudit,"sharedComputedPrimitives":"MATCH","headerHeight":mcontract['header']['height']}
