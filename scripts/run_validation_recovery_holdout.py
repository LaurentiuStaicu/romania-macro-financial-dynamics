...
SPLIT_PATH = ROOT / "data" / "provenance" / "validation_recovery_split.json"
...
def main() -> None:
    ...
    candidate = [predict_delta_policy(lending, policy, i, candidate_fit, lag=0) for i in holdout_indices]
    ...
    metrics = {"candidate": error_metrics(actual, candidate), "persistence": error_metrics(actual, persistence), "constant_policy_spread": error_metrics(actual, constant_spread)}
    candidate_rmse = float(metrics["candidate"]["rmse"])
    improvements = {baseline: 1.0 - candidate_rmse / float(metrics[baseline]["rmse"]) for baseline in ("persistence", "constant_policy_spread")}
    best_baseline_mae = min(float(metrics["persistence"]["mae"]), float(metrics["constant_policy_spread"]["mae"]))
    beta = candidate_fit.parameters["beta"]
    sensitivity = {}
    for label, factor in (("minus_10pct", 0.9), ("plus_10pct", 1.1)):
        altered_beta = beta * factor
        altered_predictions = [lending[i - 1] + altered_beta * (policy[i] - policy[i - 1]) for i in holdout_indices]
        sensitivity[label] = error_metrics(actual, altered_predictions)
    sensitivity_pass = all(float(result["rmse"]) <= 2.0 * candidate_rmse + 1e-12 for result in sensitivity.values())
    gates = {
        "minimum_observations": len(holdout_indices) >= contract["final_holdout_gates_if_opened"]["minimum_observations"],
        "rmse_vs_each_baseline": all(value >= contract["final_holdout_gates_if_opened"]["rmse_improvement_vs_each_baseline_fraction"] for value in improvements.values()),
        "mae_vs_best_baseline": float(metrics["candidate"]["mae"]) <= best_baseline_mae * (1.0 + contract["final_holdout_gates_if_opened"]["mae_may_not_exceed_best_baseline_by_more_than_fraction"]),
        "absolute_bias": abs(float(metrics["candidate"]["bias_actual_minus_predicted"])) <= contract["final_holdout_gates_if_opened"]["absolute_bias_max_percentage_points"],
        "sensitivity": sensitivity_pass,
    }
    nonzero_policy_changes_holdout = sum(1 for i in holdout_indices if abs(policy[i] - policy[i - 1]) > 1e-12)
    passes = all(gates.values())
    result = {
        "result_version": "0.1-final-evaluation",
