...
SPLIT = ROOT / "data" / "provenance" / "validation_recovery_split.json"
...
def evaluate_target(target_id: str, target_path: Path, split: dict, contract: dict) -> dict:
    ...
    calibration_idx = indices_between(periods, split["roles"]["calibration"]["start"], split["roles"]["calibration"]["end"])
    selection_idx = indices_between(periods, split["roles"]["structural_selection"]["start"], split["roles"]["structural_selection"]["end"])
    ...
    for model_id in baseline_ids:
        ...
        baseline_results[model_id] = {"metrics": metrics, "origins": len(selection_idx), "parameter_summary": parameter_summary(model_id, fits) if fits else {"n": 0}}
    ...
    for model_id in candidate_ids:
        ...
        try:
            calibration_fit = fit_model(model_id, lending, policy, calibration_observations, calibration_transitions)
        ...
        try:
            ...
            rmse_improvements = {baseline_id: improvement(float(metrics["rmse"]), float(baseline_results[baseline_id]["metrics"]["rmse"])) for baseline_id in baseline_ids}
            gates = {
                ...
                "rmse_vs_each_baseline": all(value >= contract["structural_selection_gates"]["candidate_rmse_improvement_vs_each_baseline_fraction"] for value in rmse_improvements.values()),
                "mae_vs_best_baseline": float(metrics["mae"]) <= best_baseline_mae * (1.0 + contract["structural_selection_gates"]["candidate_mae_may_not_exceed_best_baseline_by_more_than_fraction"]),
                "absolute_bias": abs(float(metrics["bias_actual_minus_predicted"])) <= contract["structural_selection_gates"]["candidate_absolute_bias_max_percentage_points"],
                "parameter_domain_consistency": float(psummary.get("domain_fraction", 0.0)) >= contract["structural_selection_gates"]["parameter_sign_or_domain_consistency_fraction"],
            }
            candidates[model_id] = {
                "calibration_fit": {"parameters": calibration_fit.parameters, "training_observations": calibration_fit.training_observations, "driver_sum_squares": calibration_fit.driver_sum_squares, "design_rank": calibration_fit.design_rank} if calibration_fit else None,
            ...
            candidates[model_id] = {"calibration_fit": None, "calibration_identifiability_error": calibration_error, "selection_identifiability_error": str(exc), "passes_all_selection_gates": False}
...
def main() -> None:
    ...
    results = {
        ...
        "previous_holdout_status": "CONTAMINATED_RECLASSIFIED_AS_STRUCTURAL_SELECTION",
        "targets": {target_id: evaluate_target(target_id, path, split, contract) for target_id, path in targets.items()},
    }
    passed = {target: result["passed_candidates"] for target, result in results["targets"].items() if result["passed_candidates"]}
    results["candidates_eligible_for_final_evaluation"] = passed
    results["open_final_holdout"] = bool(passed)
    results["selection_stage_verdict"] = "OPEN_FINAL_HOLDOUT_FOR_ELIGIBLE_TARGETS" if passed else "CLOSE_NO_GO_WITHOUT_OPENING_HOLDOUT"
