...
class CreditRiskNplSourceBoundaryTests(unittest.TestCase):
    ...
    def test_mortgage_portfolio_family_is_observed_but_not_selected(self) -> None:
        family = self.review["sector_specific_npl_family"]
        self.assertEqual(
            family["status"],
            "BNR_MORTGAGE_NPL_PUBLICATION_FAMILY_OBSERVED_NOT_SELECTED_AS_MODEL_TARGET",
        )
        self.assertEqual(family["current_target_selection"], "UNFROZEN")
        self.assertEqual(
            self.review["mortgage_portfolio_source_screening"],
    ...
    def test_dti_o_does_not_close_debt_service_boundary(self) -> None:
        dti = self.review["debt_service_boundary"]["dti_o_boundary"]
        self.assertEqual(dti["status"], "OBSERVED_BUT_NOT_DEBT_SERVICE_BURDEN")
        self.assertEqual(dti["reported_mortgage_exposure_value_coverage_percent"], 25)
        self.assertIn("may not substitute", dti["rule"])

    def test_future_reopen_paths_keep_aggregate_and_mortgage_targets_separate(self) -> None:
        paths = {
            item["id"]: item
            for item in self.review["source_admissibility"]["future_reopen_paths"]
        }
        self.assertFalse(paths["AGGREGATE_TARGET_PATH"]["target_switch"])
        self.assertTrue(
            paths["MORTGAGE_PORTFOLIO_PATH"]["target_switch_requires_new_contract"]
        )
